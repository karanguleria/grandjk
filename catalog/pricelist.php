...
if ($colour_id) {
    ...
    {
       $categories_sql = 'SELECT * FROM categories where `state`= 1 ORDER BY orderid';
    }
...
if ($categories) {
    foreach ($categories as $category) {
        echo ' / <a href="#' . $category['category'] . '">' . stripslashes(ucwords(strtolower($category['category']))), '</a>';
    }
}
            ...
            foreach ($categories as $category) {
                ...
                if ($sub_categories) {
                    foreach ($sub_categories as $sub_category) {
                        ...
                        ?>
                        <?php if (isset($prices[$sub_category['sub_category_id']])) { ?>
